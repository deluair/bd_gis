...
- **Coverage end dates differ by product**: WorldPop ends 2020, LandScan runs to 2024, GRACE mascon ends 2017, Sentinel-5P starts late 2018, Dynamic World starts 2015.
- **Population grids are not interchangeable: `measure` says which.** WorldPop is `residential` (where people sleep, census-disaggregated, 100 m). LandScan is `ambient` (24-hour average of where people actually are, including work and transit, ~928 m). Ambient is the right basis for exposure, which does not wait for people to be home; residential is the right basis for dwellings and households. Never build a time series or a ratio that mixes them. Both are counts per cell, so both carry the scale hazard below.
- **A count grid must be reduced at its own native scale.** WorldPop is people per 100 m cell and LandScan people per ~928 m cell. Summing either at the wrong scale rescales the answer (summing WorldPop at 1 km undercounts ~100x). `coastal.get_population_grid` returns the image and its native scale together so the two cannot drift apart.
- **Masking a count grid with a finer mask is all-or-nothing, and that is a real bias, not a rounding error.** A 928 m LandScan cell is kept whole or dropped whole against a 30 m SRTM mask. Measured over `COASTAL_BOUNDS` for 2020, naive masking undercounts LandScan by 65% (2.86M vs 8.21M) and WorldPop by 10% (6.27M vs 6.94M). Use `coastal.lecz_coverage_fraction` (fractional coverage via `reduceResolution`, then multiply) for any exposure count against a fine mask. Confirmed by control: after area weighting the LandScan/WorldPop ratio inside the LECZ (1.182) converges on their ratio over the whole region (1.216), so the naive 0.457 ratio was purely an artifact.
- **LandScan's operational annual releases are NOT a consistent panel.** The Bangladesh national total falls 7.52% from 2015 (168.72M) to 2016 (156.03M), 12.7M people who did not disappear. LandScan re-runs its model each year on the best inputs then available and does not reprocess history. ORNL: "users should avoid conducting change analysis at the cell level". Treat the 2015/2016 boundary exactly like the DMSP/VIIRS 2013/2014 boundary. Within 2000-2015 and within 2016-2024 the series is smooth (+0.2% to +3.4% a year). For genuine change over time use the reprocessed consistent 2000-2022 series (doi:10.1038/s41597-025-04817-z), not this collection.
- **`.median()` and other ee reducers drop the projection.** A composite's `.projection()` returns the default WGS84 1-degree grid, not the source grid, which makes `reduceResolution`/`reproject` fail or silently misbehave. Pin projections explicitly, e.g. `ee.Projection("EPSG:4326").atScale(scale)`.
...
| # | Severity | Date | Issue |
|---|----------|------|-------|
| 0e | Low | 2026-08-16 | **`poverty.py`, `transportation.py` and `nightlights.py` still clamp silently to WorldPop's 2020 ceiling**, so asking any of them for 2024 quietly analyses 2020 population. They were deliberately NOT migrated to LandScan when it was wired in: (a) the poverty composite is under open issue 0a and swapping an input mid-investigation would confound it; (b) `transportation.py` normalises with `pop.unitScale(0, 5000)` and `nightlights.py` divides by population, both tuned to WorldPop's people-per-100 m-cell units, so switching to ~928 m cells changes every published index value and the constants would need re-deriving; (c) any switch mixes ambient with residential across years. Migrating them is a real task with real re-calibration, not a find-and-replace. Until then the silent clamp is the more visible risk: consider raising rather than clamping, as `coastal.get_population_grid` now does. |
...
| Date | Item |
|------|------|
| 2026-08-16 | **`compute_lecz_population` was undercounting by ~10%, and the bug scaled with cell size.** It masked a population count grid with a 30 m SRTM LECZ mask and summed, which keeps or drops each population cell whole. Over a fragmented delta fringe that biased the published `lecz_population.csv` figure low by 10.7% on WorldPop (6.27M -> 6.94M for 2020) and would have biased LandScan low by 65% (2.86M -> 8.21M). Replaced with area-weighted fractional coverage (`lecz_coverage_fraction`). Control check: post-fix the LandScan/WorldPop ratio inside the LECZ (1.182) matches their ratio over the whole region (1.216), confirming the residual difference is between the grids and not the masking. The absolute level still inherits SRTM's known positive elevation bias over vegetated delta, which shrinks a 5 m zone; that bias is NOT corrected here and the level is not independently validated. |
| 2026-08-16 | **LandScan Global wired in as an explicit second population source** (`cfg.LANDSCAN`, CC BY 4.0, ORNL, doi:10.48690/1532445). Verified live against Earth Engine: 25 images `landscan-global-2000` to `landscan-global-2024`, band `b1`, nominalScale 927.662423277276 m, EPSG:4326, Bangladesh sums 128.8M (2000) to 168.3M (2024). The Google Earth Engine catalog page states 1000 m and a 2023 end year; both are stale, do not trust it over the asset. The asset is the Samapriya Roy community mirror, not an official ORNL or Google asset, so expect it to lag the ORNL portal and cite ORNL rather than the mirror. `compute_lecz_population` now defaults to LandScan; `poverty.py`, `transportation.py` and `nightlights.py` were deliberately left on WorldPop (see Open issues). |
