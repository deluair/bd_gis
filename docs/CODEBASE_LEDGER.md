...
| # | Severity | Date | Issue |
|---|----------|------|-------|
| 0a | **High** | 2026-08-01 | **The `gis_poverty_index` validation card is not reproducible from the pipeline's own output.** `outputs/validation/gis_poverty_index.json` claims tier A, `pearson_r = 0.99957`, `n = 8`, `caveats: []`. But `outputs/poverty/poverty_by_division.csv` contains 7 divisions with `poverty_index_mean` spanning only 0.5025 to 0.5127 (a spread of 0.010) against HIES HCR spanning 10.8% to 24.8%. Recomputed Pearson r against `hies_division_hcr.csv` is **+0.449**, which is tier C ("weak or uncalibrated"), not tier A. Whatever CSV was passed to `--predicted-csv` was not the poverty module's output (card bias of -19.29 implies mean predicted ≈ 0.10, versus 0.509 in the module output). Until this is reconciled, the tier A poverty claim should not be cited anywhere. |
| 0b | **High** | 2026-08-01 | `poverty_by_division.csv` covers 7 divisions and omits **Mymensingh**, which HIES 2022 reports as the poorest division at 28.7% HCR. Any division ranking built from this file silently drops the worst-off division. The card nonetheless reports `n = 8`. |
